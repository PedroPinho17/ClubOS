...
const CardFrame = forwardRef<HTMLDivElement, { width: number; children: React.ReactNode }>(
...
function Fields({ data, dense = false }: { data: CardData; dense?: boolean }) {
  ...
  return (
    <div style={{ lineHeight: 1.15 }}>
      <div style={{ fontSize: 20, letterSpacing: 2, opacity: 0.8 }} className="uppercase">
        {l.cardTitle}
      </div>
      {l.showNome && (
        <div style={{ fontSize: dense ? 34 : 40, fontWeight: 800, marginTop: 4 }}>{data.member.name}</div>
      )}
      {l.showCargo && data.member.cardRole && (
        <div style={{ fontSize: 20, marginTop: 6, opacity: 0.9 }}>
          <span style={{ opacity: 0.7 }}>{l.cargoLabel}:</span> {data.member.cardRole}
        </div>
      )}
      {l.showNumero && (
        <div style={{ fontSize: 24, marginTop: 8, fontWeight: 600 }}>N.º {data.numeroFormatado}</div>
      )}
      {l.showPlano && data.member.planName && (
        <div style={{ fontSize: 18, marginTop: 4, opacity: 0.85 }}>{data.member.planName}</div>
      )}
      {l.showEmail && data.member.email && (
        <div style={{ fontSize: 16, marginTop: 4, opacity: 0.8 }}>{data.member.email}</div>
      )}
      {l.showTelefone && data.member.phone && (
        <div style={{ fontSize: 16, marginTop: 2, opacity: 0.8 }}>{data.member.phone}</div>
      )}
      {l.showValidade && data.validityText && (
        <div style={{ fontSize: 16, marginTop: 8, opacity: 0.75 }}>{data.validityText}</div>
      )}
    </div>
  );
}
...
function LogoBlock({ data, size = 26 }: { data: CardData; size?: number }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center">
      <div style={{ fontSize: size, fontWeight: 800, letterSpacing: 1 }}>{data.organization.name}</div>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col justify-center pl-6">
        <Fields data={data} />
        {l.footerText && <div style={{ fontSize: 14, marginTop: 10, opacity: 0.7 }}>{l.footerText}</div>}
      </div>
      <div style={{ position: 'absolute', bottom: 22, right: 22, color: l.accentColor }}>
...
function ModernCard({ data }: { data: CardData }) {
  ...
  return (
    ...
    >
      ...
      {l.footerText && (
        <div style={{ fontSize: 14, opacity: 0.7 }} className="absolute bottom-4 left-8">
          {l.footerText}
        </div>
      )}
    </div>
...
function MinimalCard({ data }: { data: CardData }) {
  ...
  return (
    ...
    >
      ...
      {l.footerText && <div style={{ fontSize: 14, opacity: 0.7 }}>{l.footerText}</div>}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', paddingTop: 18, color: navy }}>
        <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: 2 }}>CLUBE RECREATIVO</div>
        <div style={{ fontSize: 44, fontWeight: 900, lineHeight: 1, color: blue }}>
          {data.organization.name.toUpperCase().includes('VALE') ? 'VALE' : data.organization.name.toUpperCase()}
        </div>
        <div style={{ fontSize: 13, letterSpacing: 3, color: navy, opacity: 0.7, marginTop: 2 }}>
      ...
      >
        <div className="flex gap-4">
          <Photo data={data} size={120} />
          <div>
            <div style={{ fontSize: 16, letterSpacing: 1, color: blue, fontWeight: 700 }}>CARTÃO SÓCIO</div>
            <div style={{ fontSize: 18, marginTop: 2 }}>{data.member.cardRole ?? data.layout.cardTitle}</div>
          </div>
        </div>
        <div style={{ marginTop: 16 }}>
          <div style={{ fontSize: 14, color: blue }}>NOME</div>
          <div style={{ fontSize: 26, fontWeight: 800, textTransform: 'uppercase' }}>{data.member.name}</div>
        </div>
        <div className="flex gap-10" style={{ marginTop: 12 }}>
          <div>
            <div style={{ fontSize: 14, color: blue }}>MATRÍCULA</div>
            <div style={{ fontSize: 20, fontWeight: 700 }}>{data.numeroFormatado}</div>
          </div>
          <div>
            <div style={{ fontSize: 14, color: blue }}>VALIDADE</div>
            <div style={{ fontSize: 20, fontWeight: 700 }}>{data.validadePeriodo}</div>
          </div>
        </div>
        <div style={{ marginTop: 14 }} className="flex items-center gap-3">
          ...
          <div style={{ fontStyle: 'italic', fontSize: 18, color: sky }}>{data.layout.slogan}</div>
        </div>
      </div>

      {/* Coluna direita: crest + badge */}
      <div
        style={{ position: 'absolute', top: 140, right: 26, width: '30%', textAlign: 'center', color: navy }}
      >
        <div
          style={{
            width: 120,
        ...
        />
        <div style={{ fontSize: 12, letterSpacing: 2, marginTop: 8 }}>SÓCIO VERIFICADO ✓</div>
      </div>
