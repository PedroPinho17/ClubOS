...
test.describe("Comunicacoes", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Comunicações", exact: true }),
    ).toBeVisible();
  });

  test("preview de email e historico", async ({ page }) => {
    await page
      .locator('label:text-is("Assunto")')
      .locator("..")
      .locator("input")
      .fill("Teste E2E Comunicações");
    ...
    await expect(page.getByText("Pré-visualização do email")).toBeVisible({
      timeout: 15_000,
    });
    await expect(page.getByText("João Exemplo")).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Histórico (email)" }),
    ).toBeVisible();
  });
