...
import { selectMemberInPaymentsForm } from "./helpers";
...
test.describe("Pagamentos", () => {
  ...
  test("regista pagamento para socio existente", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Registar pagamento" }),
    ).toBeVisible({ timeout: 20_000 });

    await expect(
      page.locator("tbody").getByText("12.50 €").first(),
    ).toBeVisible({ timeout: 15_000 });
    await expect(
      page.locator("tbody").getByText("Transferência").first(),
    ).toBeVisible();
  });
});
