  ...
  async invite(
    ...
    if (actorRole === "administrador" && dto.role !== "tesoureiro") {
      throw new ForbiddenException(
        "Administradores so podem convidar tesoureiros.",
      );
    }
    ...
    if (existing?.role === "socio" && existing.member) {
      throw new BadRequestException(
        "Este email esta associado a um socio. Use o portal do socio.",
      );
    }

    const alreadyInOrg = existing?.organizationMemberships.some(
      (m) => m.organizationId === organizationId,
    );
    if (alreadyInOrg) {
      throw new BadRequestException(
        "Este utilizador ja pertence a esta organizacao.",
      );
    }

    const otherOrgMembership = existing?.organizationMemberships.find(
      (m) => m.organizationId !== organizationId,
    );
    if (otherOrgMembership && actorRole !== "imperador") {
      throw new BadRequestException(
        "Este email ja pertence a outra organizacao.",
      );
    }
